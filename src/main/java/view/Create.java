...
//					
//					errors.put(FIELD_EMAIL, "cet email est d�j� enregistr�");
...
//		}
	...
	private void validateForm(String profil, String email, String nom,
		...
		if (profil.isEmpty()) {
			errors.put(FIELD_PROFIL, "La s�lection d'un profil est obligatoire");
		}
		if (fumeur == null) {
		...
		if (genre == null) {
			errors.put(FIELD_GENRE, "La s�lection du genre est obligatoire");
		}
		if (email.isEmpty()) {
		...
		if (prenom.isEmpty()) {
			errors.put(FIELD_PRENOM, "Le pr�nom est obligatoire");
		} 
